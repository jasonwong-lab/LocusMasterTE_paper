...
age_process <- function(combined_age){
  combined_age <- combined_age[which(combined_age$short != 0 | combined_age$LocusMasterTE != 0),]
  combined_age$diff_raw <- as.numeric(combined_age$short) - as.numeric(combined_age$LocusMasterTE)
...
correlation <- function(combined_dataset,cell_line_name){
  ...
  p2 <- p2 + ggtitle("LocusMasterTE") + xlab("log(TPM by LocusMasterTE)") + ylab("log(TPM by Long Read)")
}
...
histone_own_HCT116$diff <- histone_own_HCT116$LocusMasterTE - histone_own_HCT116$short
